...
the `translate.py` script, which prompts the user to input the challenge text,
and then prompts again for every unknown symbol -- displaying all of the
...
Continuing the recon, I pasted the whole base64 blob into a text file, then
attempted to decode and analyze it:
